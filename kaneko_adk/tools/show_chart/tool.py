...
from kaneko_adk.tools.show_chart.auto_generated_gemini_models import Model, Type
...
class ShowChartTool(FunctionTool):
    ...
    async def run_async(
        ...
        if (
            inspect.iscoroutinefunction(self.func) or
            hasattr(self.func, '__call__') and
            inspect.iscoroutinefunction(self.func.__call__)
        ):
